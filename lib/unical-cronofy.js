...
class CronofyConnector extends EventEmitter {
	 ...
	 * @param {function (error: any, result: any)} callback
	 */
	revokeAuthCredentials(auth, callback) {
		...
		token.revoke('access_token', (err, accessTokenRevocation) => {
			if (err) {
				return callback(err, null);
			}
			token.revoke('refresh_token', (err, refreshTokenRevocation) => {
				if (err) {
					return callback(err, null);
				}

				return callback(null, { accessTokenRevocation, refreshTokenRevocation });
