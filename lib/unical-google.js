...
class GoogleConnector extends EventEmitter {
    ...
    watchEvents(auth, params, options, callback) {
        ...
        return this._prepareApiCall({ credentials: auth, oauth: null }, (err, result) => {
            ...
            try {
                ...
                GoogleCal.events.watch(googleCalParams, (err, notificationResponse) => {
                    ...
                    const uniformChannelId = `${notificationResponse.data.id}///${data.resourceId}`;
                    return callback(null, { channelId: uniformChannelId });
                });
            }
            catch(exception) {
                return callback(exception, null);
        ...
        return this._prepareApiCall({ credentials: auth, oauth: null }, (err, result) => {
            ...
            try {
                ...
                if (parsedData?.length !== 2) {
                    throw new Error(`Calendar ID is in an invalid format; '${params.calendarId}'.`);
                }
                ...
                GoogleCal.channels.stop(googleCalParams, (err, res) => {
    ...
    _parseFromDateFromOriginalParams(fromDate) {
        if (!fromDate) {
            return DateFns.subDays(Date.now(), 42);
        } else {
            return DateFns.max([DateFns.subDays(Date.now(), 42), fromDate]);
        }
    }
    ...
    _parseToDateFromOriginalParams(toDate) {
        if (!toDate) {
            return DateFns.addDays(Date.now(), 201);
        } else {
            return DateFns.min([DateFns.addDays(Date.now(), 201), toDate]);
        }
    }
}
